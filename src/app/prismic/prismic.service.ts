...
export class PrismicService {
  ...
  buildContext() {
    ...
    return Prismic.api(CONFIG.apiEndpoint, { accessToken: CONFIG.accessToken })
      ...
      .catch(e =>
        console.log(`Error during connection to your Prismic API: ${e}`)
      );
  }
  ...
  validateOnboarding() {
    ...
    if (infos.isConfigured) {
      this.http
        .post(`${infos.repoURL}/app/settings/onboarding/run`, null)
        .subscribe(null, err =>
          console.log(
            `Cannot access your repository, check your api endpoint: ${err}`
          )
        );
    }
  }
  ...
  toolbar(api) {
    ...
    if (maybeCurrentExperiment) {
      window['PrismicToolbar'].startExperiment(
        maybeCurrentExperiment.googleId()
      );
    }
  ...
  preview(token) {
    return this.buildContext().then(ctx => {
